    ...
    string AESL_num;
{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<24> > a_pc_buffer(3);
          int i = 0;
          ...
          if (i > 0) {{
		    int i = 0;
            for (int j = 0, e = 3; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_a)[j*4+0] = a_pc_buffer[i].range(7, 0).to_int64();
((char*)__xlx_apatb_param_a)[j*4+1] = a_pc_buffer[i].range(15, 8).to_int64();
((char*)__xlx_apatb_param_a)[j*4+2] = a_pc_buffer[i].range(23, 16).to_int64();
}}}
        } // end transaction
      } // end file is good
...
for (int i = 0; i < 3; ++i) {
auto *pos = (unsigned char*)__xlx_apatb_param_a+i*4;
aesl_fh.write(AUTOTB_TVIN_a, formatData(pos, 24));
}
}
...
for (int i = 0; i < 3; ++i) {
auto *pos = (unsigned char*)__xlx_apatb_param_b+i*4;
aesl_fh.write(AUTOTB_TVIN_b, formatData(pos, 24));
}
}
...
for (int i = 0; i < 9; ++i) {
auto *pos = (unsigned char*)__xlx_apatb_param_res+i*2;
aesl_fh.write(AUTOTB_TVIN_res, formatData(pos, 16));
}
}
...
for (int i = 0; i < 3; ++i) {
auto *pos = (unsigned char*)__xlx_apatb_param_a+i*4;
aesl_fh.write(AUTOTB_TVOUT_a, formatData(pos, 24));
}
}
...
for (int i = 0; i < 3; ++i) {
auto *pos = (unsigned char*)__xlx_apatb_param_b+i*4;
aesl_fh.write(AUTOTB_TVOUT_b, formatData(pos, 24));
}
}
...
for (int i = 0; i < 9; ++i) {
auto *pos = (unsigned char*)__xlx_apatb_param_res+i*2;
aesl_fh.write(AUTOTB_TVOUT_res, formatData(pos, 16));
}
}
